...
// Owns the sync detector plus a tracker wrapping it (tracker only holds a
// reference, so the detector must outlive it: keeping both together in one
// heap-allocated struct makes that lifetime trivially correct). Calling into
// the tracker stays fast per call: the ONNX inference itself runs on the
// tracker's own internal background thread, so this is safe to call once per
// analyzed camera frame without blocking the caller on inference latency.
struct NativeDetector {
  card_detection::CardDetector detector;
  card_tracking::DetectionTracker tracker;

  NativeDetector(const std::string &corner_weights,
                 const std::string &class_weights)
      : detector(corner_weights, class_weights, /*device_id=*/-1),
        tracker(detector) {}
};
...
jobject color_to_jobject(JNIEnv *env, jclass color_class, set_game::Color c) {
  ...
  jfieldID id = env->GetStaticFieldID(
      color_class, name, "Lcom/fabianleven/setdetect/domain/CardColor;");
...
jobject shape_to_jobject(JNIEnv *env, jclass shape_class, set_game::Shape s) {
  ...
  jfieldID id = env->GetStaticFieldID(
      shape_class, name, "Lcom/fabianleven/setdetect/domain/CardShape;");
...
jobject count_to_jobject(JNIEnv *env, jclass number_class, set_game::Count c) {
  ...
  jfieldID id = env->GetStaticFieldID(
      number_class, name, "Lcom/fabianleven/setdetect/domain/CardNumber;");
...
jobject fill_to_jobject(JNIEnv *env, jclass shading_class, set_game::Fill f) {
  ...
  jfieldID id = env->GetStaticFieldID(
      shading_class, name, "Lcom/fabianleven/setdetect/domain/CardShading;");
...
Java_com_fabianleven_setdetect_domain_NativeSetDetector_initNative(
    JNIEnv *env, jobject /* this */, jstring corner_model_path,
    jstring class_model_path) {
...
Java_com_fabianleven_setdetect_domain_NativeSetDetector_closeNative(
    JNIEnv * /* env */, jobject /* this */, jlong native_ptr) {
  delete reinterpret_cast<NativeDetector *>(native_ptr);
}

extern "C" JNIEXPORT jobject JNICALL
Java_com_fabianleven_setdetect_domain_NativeSetDetector_detectNative(
  ...
  const cv::Mat rgba_mat(static_cast<int>(info.height),
                         static_cast<int>(info.width), CV_8UC4, pixels,
                         info.stride);
  cv::Mat img_rgb;
  cv::cvtColor(rgba_mat, img_rgb, cv::COLOR_RGBA2RGB);
  AndroidBitmap_unlockPixels(env, bitmap);

  const std::optional<card_detection::Detection> detection =
      native->tracker.detect(img_rgb);
  ...
  jclass det_card_class =
      env->FindClass("com/fabianleven/setdetect/domain/DetectedCard");
  jmethodID det_card_init = env->GetMethodID(
      det_card_class, "<init>",
      "(Lcom/fabianleven/setdetect/domain/SetCard;Ljava/util/List;F)V");

  jclass set_card_class =
      env->FindClass("com/fabianleven/setdetect/domain/SetCard");
  jmethodID set_card_init =
      env->GetMethodID(set_card_class, "<init>",
                       "(Lcom/fabianleven/setdetect/domain/CardColor;Lcom/"
                       "fabianleven/setdetect/domain/CardShape;"
                       "Lcom/fabianleven/setdetect/domain/CardNumber;Lcom/"
                       "fabianleven/setdetect/domain/CardShading;)V");

  jclass color_class =
      env->FindClass("com/fabianleven/setdetect/domain/CardColor");
  jclass shape_class =
      env->FindClass("com/fabianleven/setdetect/domain/CardShape");
  jclass number_class =
      env->FindClass("com/fabianleven/setdetect/domain/CardNumber");
  jclass shading_class =
      env->FindClass("com/fabianleven/setdetect/domain/CardShading");

  jclass point_class =
      env->FindClass("com/fabianleven/setdetect/domain/PointF");
  ...
  for (const card_detection::DetectedCard &dc : detection->detected_cards) {
    ...
    if (const std::optional<set_game::Card> card =
            card_detection::match_card(dc)) {
      ...
      if (jcolor != nullptr && jshape != nullptr && jnumber != nullptr &&
          jshading != nullptr) {
        jcard = env->NewObject(set_card_class, set_card_init, jcolor, jshape,
                               jnumber, jshading);
      }
    }
    ...
    for (const card_detection::Point2d &corner : dc.corners) {
      jobject jp =
          env->NewObject(point_class, point_init, static_cast<float>(corner.x),
    ...
    jobject jdet =
        env->NewObject(det_card_class, det_card_init, jcard, jcorners, conf);
    env->CallBooleanMethod(det_cards_list, list_add, jdet);
  }

  const auto &arr = detection->matches_arrangement;
  jclass rect_class =
      env->FindClass("com/fabianleven/setdetect/domain/RotatedRect");
  ...
  for (const auto &pose : arr.card_poses) {
    jobject jpose = env->NewObject(
        rect_class, rect_init, static_cast<float>(pose.center_x),
  ...
  jclass arrangement_class =
      env->FindClass("com/fabianleven/setdetect/domain/CardsArrangement");
  jmethodID arrangement_init = env->GetMethodID(
      arrangement_class, "<init>", "(Ljava/util/List;Ljava/util/List;FF)V");
  jobject arrangement_obj = env->NewObject(
      arrangement_class, arrangement_init, poses_list, z_orders_list,
      static_cast<float>(arr.card_width), static_cast<float>(arr.card_height));

  jclass detection_class =
      env->FindClass("com/fabianleven/setdetect/domain/Detection");
  jmethodID detection_init = env->GetMethodID(
      detection_class, "<init>",
      "(Ljava/util/List;Lcom/fabianleven/setdetect/domain/CardsArrangement;)V");

  return env->NewObject(detection_class, detection_init, det_cards_list,
                        arrangement_obj);
}
