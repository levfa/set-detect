...
// What cv::buildOpticalFlowPyramid produces: an image + Scharr-derivative pyramid,
// opaque to callers, passed straight into cv::calcOpticalFlowPyrLK's
// pyramid-accepting overload. Building it once per frame and reusing it for both the
// forward and backward LK pass avoids rebuilding it twice per direction.
//
// tryReuseInputImage means level 0 may be a zero-copy view over the cv::Mat this
// pyramid was built from; that Mat must outlive this pyramid.
...
// Same as calc_optical_flow_pyr_lk but on caller-built pyramids; the levels are
// treated as read-only so a single pair can serve both flow directions. threads,
// when > 0, temporarily overrides cv::setNumThreads() for the duration of this call
// (cv::calcOpticalFlowPyrLK parallelizes internally via cv::parallel_for_); -1
// (default) leaves the process-wide setting alone. cv::setNumThreads is
// process-wide, not scoped to this call or thread, so passing threads > 0 while
// another thread concurrently runs an OpenCV parallel operation can transiently
// change that operation's parallelism too.
...
// Thin, stateless wrapper around calc_optical_flow_on_pyramids for callers that
// prefer to hold an instance across frames rather than call the free function
// directly. cv::calcOpticalFlowPyrLK needs no persistent worker pool or scratch
// state of its own (OpenCV parallelizes internally), so holding an instance carries
// no benefit beyond that calling convention (per-call construction cost is
// negligible here).
