...
// modes depend solely on from_frame_id, shared by every card in one Detection, so this
// is behaviorally identical to propagating each card separately). On success, rebuilds
// detected_cards with the new corners (visibility/probs unchanged) and rebuilds matches
// by re-running match_card() against the rebuilt cards, so matches' paired DetectedCard
// also carries the propagated corners. On failure (or an empty detection), returns an
// otherwise-empty Detection. matches_arrangement is always carried over unchanged --
// only a fresh detection recomputes the arrangement, never a propagated one.
...
// every frame at video pace while the (slower) detector only actually completes every
...
class DetectionTracker {
  public:
    // max_side: detect() resizes an incoming frame down so its longest side is at most
